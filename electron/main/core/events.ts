...
import { setTrayLanguage } from './trayI18n'
...
export function setupIpcEvents(window: BrowserWindow | null) {
  ...
  ipcMain.on(IpcChannels.UpdateLanguage, async (_, language: string) => {
    await setTrayLanguage(language)
    updateTray()
  })
}
